...
const items=[{id:'festival',title:'双节福利',genre:'节日活动',synopsis:'',image:festivalAssets.desktop,focus:'center'},...adultBanners];
export function AdultHero() {
 ...
 return <section className={`adult-cinema adult-cinema-artwork ${item.id==='festival'?'adult-festival-slide':''}`} aria-label="重点推荐" aria-roledescription="轮播" onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} onFocusCapture={()=>setFocus(true)} onBlurCapture={e=>{if(!e.currentTarget.contains(e.relatedTarget))setFocus(false);}}>
