...
export function AdultLayout({children, qualified=false}:{children:ReactNode;qualified?:boolean}) {
 ...
 const [dialog,setDialog]=useState<'privacy'|'clear'|'member'|null>(null),[message,setMessage]=useState('');
 const [query,setQuery]=useState(''),[committed,setCommitted]=useState('');
 const tab=route.params.get('tab')||'home';
 return <AdultSearchContext.Provider value={{query,setQuery,committed,setCommitted}}><Header brandBadge={<span className="brand-zone-mark" aria-label="18+专区"><b>18<sup>+</sup></b><i>专区</i></span>} query={query} onSearch={setQuery} onSubmitSearch={()=>{setCommitted(query.trim());navigate('/18plus/search');}} onAccount={()=>navigate(`/account/login?returnTo=${encodeURIComponent(route.path+route.search+route.hash)}`)} navigation={adultTabs.map(([id,label])=>[`/18plus?tab=${id}`,label] as const)} isActive={href=>route.path==='/18plus'&&new URL(href,location.origin).searchParams.get('tab')===tab} extraActions={<Link className="account-button" href="/18plus/wishlist" aria-label="愿望榜"><Heart size={18}/><span>愿望榜</span></Link>}/><div className="adult-shell" data-dialog-open={!!dialog}>
  {children}
