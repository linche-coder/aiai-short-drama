...
import {festivalConfig,invitationMaximum,festivalDates,festivalOffers,extraViewingDaysFor,festivalRechargePath,festivalRewardNames} from '../services/festivalModel';
...
function FestivalAccount({userId}:{userId:string|null}){
 ...
 return <main ref={root} className="festival-page">
  ...
  <section className="festival-rules" id="festival-rules"><h2>活动规则</h2><ol>
   ...
   <li>活动期间成功开通或续费畅看月卡、季卡，每笔订单均可获得对应观看天数，不限制次数。</li>
   <li>{festivalOffers.map(offer=>offer.name+'加赠'+extraViewingDaysFor(offer.id)+'天').join('，')}，赠送天数计入会员有效期。</li>
   <li>会员活动以订单支付成功时间为准，普通积分包不参加。</li>
   <li>未支付、支付失败、取消或关闭的订单不赠送观看天数。</li>
   <li>全额退款订单对应的赠送观看天数会被撤回。</li>
   <li>参与、邀请等积分奖励进入统一积分余额，永久有效，可按现有规则使用。</li>
