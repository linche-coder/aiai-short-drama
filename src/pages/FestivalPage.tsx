...
function FestivalAccount({userId}:{userId:string|null}){
 ...
 return <main ref={root} className="festival-page">
  ...
  <div className="festival-tasks">
   ...
   <section className="festival-card festival-recharge-task"><div className="festival-card-top"><span className="festival-step">双节会员充值礼</span><Crown size={25}/></div><h2>开通或续费，每笔都送积分</h2><p>月卡、年卡均参加，每笔成功订单均享加赠。</p><div className="festival-bonus-grid">{festivalOffers.map(offer=><div key={offer.id}><span>{offer.name}</span><strong>+{rechargeBonusFor(offer.id)}<small>积分</small></strong></div>)}</div><Link className="festival-secondary" href={festivalRechargePath}>查看双节充值礼 <ArrowUpRight size={17}/></Link><p className="festival-task-foot">支付成功后一次性到账，会员每月积分照常发放</p></section>
  </div>
