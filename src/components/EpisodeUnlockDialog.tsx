...
export function EpisodeUnlockDialog({contentId,episodeId,title,price,returnTo,onClose,onUnlocked}:{contentId:string;episodeId:string;title:string;price:number;returnTo:string;onClose:()=>void;onUnlocked:()=>void}){
 ...
 const cost=summary?discountedCost(price,summary.tier):price,insufficient=!!summary&&summary.balance<cost;
 ...
 {guest?<p>登录后可使用积分解锁本集。</p>:state.error?<p role="alert">{state.error}</p>:!summary?<p role="status">正在加载积分…</p>:<><p>{summary.tier==='premium'?<>原价：{price}积分<br/>尊享价：{cost}积分</>:<>本集需要 {cost} 积分</>}<br/>当前积分：{summary.balance}{insufficient&&<>，还差{cost-summary.balance}积分</>}<br/>解锁后可永久观看</p></>}
