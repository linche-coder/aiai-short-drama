...
type Entry={aiai?:{scroll:number;restore:boolean;home?:HomeState};playOrigin?:string};
...
const read=():Route=>({path:location.pathname,search:location.search,params:new URLSearchParams(location.search),hash:location.hash,key:performance.now(),restore:(history.state as Entry)?.aiai?.restore??false,scroll:(history.state as Entry)?.aiai?.scroll??0});
const isPrivate=(path:string)=>path.startsWith('/18plus');
function storeScroll(){const home=!isPrivate(location.pathname)?readHomeState(location.pathname):undefined;history.replaceState({...history.state,aiai:{scroll:isPrivate(location.pathname)?0:scrollY,restore:true,...(home?{home:{...home,scroll:scrollY,hash:location.hash}}:{})}},'');}
export function Router({children}:{children:ReactNode}){
 ...
 useLayoutEffect(()=>{
  if(isPrivate(route.path)){window.scrollTo({top:0,behavior:'instant'});return;}
 ...
 const navigate=(to:string,raw:boolean|Options=false)=>{
  ...
  const entry={...(/^\/(?:18plus\/)?play\//.test(url.pathname)&&isList?{playOrigin:source}:{}),aiai:{scroll:isPrivate(url.pathname)?0:scroll,restore:!!options.restore||!!options.preserveScroll,...(!isPrivate(url.pathname)?{home:readHomeState(url.pathname)}:{})}};
