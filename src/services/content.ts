...
export const contentService={
 ...
 async list(context:AccessContext,signal?:AbortSignal):Promise<Content[]> {
  ...
  if(!import.meta.env.DEV)throw new Error('内容服务尚未接入');
  const { adultPreview }=await import('../dev/adultPreview');
 ...
 async campaigns(context:AccessContext,signal?:AbortSignal){if(context.zone!=='adult'||!await accessService.verify(signal)||!import.meta.env.DEV)return [];const{adultCampaigns}=await import('../dev/adultPreview');if(signal?.aborted||!accessService.isGranted())return [];return visibleCampaigns(adultCampaigns,context);},
