...
export async function simulatePublish(draft:Partial<Content>):Promise<PublishResult>{
 ...
 if(!import.meta.env.DEV||accountService.getSnapshot().role!=='preview-admin')return{ok:false,issues:[{field:'role',message:'需要后台角色；会员身份不能授权此操作'}],message:'无权执行',simulation:true};
