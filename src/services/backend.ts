...
export interface MembershipSnapshotDTO {level:number;growth:number;nextLevelGrowth:number;expiresAt:string|null;permanent?:boolean}
...
export interface Backend {
 festival():Promise<FestivalData>;
 festivalClaim():Promise<FestivalData>;
 festivalReferral(code:string):Promise<{bound:boolean}>;
 points():Promise<PointsData>;
 meOverview():Promise<MeOverviewDTO>;
 checkIn():Promise<PointsData>;
 redeemMember(tierId:string,idempotencyKey:string):Promise<PointsData>;
 claimTrial():Promise<MeOverviewDTO>;
 changePassword(input:{currentPassword:string;newPassword:string}):Promise<void>;
 unlockEpisode(contentId:string,episodeId:string,idempotencyKey:string,quotedCost?:number):Promise<PointsData>;
 pointsPurchase(offerId:string,idempotencyKey:string):Promise<PointsData>;
