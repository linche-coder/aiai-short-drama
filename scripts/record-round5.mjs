...
const out=path.resolve('docs/round-5');
const browser=await chromium.launch({channel:'chrome'});
async function capture(name,viewport,mobile,scenes){
 const folder=path.resolve(`.local/round5-${name}`);await fs.mkdir(folder,{recursive:true});
...
await capture('desktop',{width:1920,height:1080},false,async(page,mark)=>{
 ...
 await page.getByRole('button',{name:'悬疑',exact:true}).click();await page.waitForTimeout(500);await mark('悬疑筛选');await page.getByRole('navigation').getByText('最新',{exact:true}).click();await page.waitForTimeout(1100);await page.screenshot({path:path.join(out,'desktop-latest.png')});await mark('恢复首页最新分区');
 await page.getByRole('textbox').fill('月色');await page.waitForTimeout(500);await page.locator('.drama-card').scrollIntoViewIfNeeded();await page.screenshot({path:path.join(out,'desktop-one-result.png')});await page.locator('.drama-card').click();await page.waitForTimeout(500);await page.screenshot({path:path.join(out,'desktop-details.png')});await page.goBack();await page.waitForTimeout(400);await mark('返回恢复搜索');
});
