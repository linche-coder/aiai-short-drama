...
test('atomic participation, concurrent requests, retries and persisted recovery',async()=>{
 ...
 expect(results.filter(r=>r.awarded)).toHaveLength(1);expect(results.every(r=>r.totalReward===20)).toBeTruthy();
 const wallet=await data(api,'/api/v1/me/points');expect(wallet.summary.balance-before.summary.balance).toBe(20);expect(wallet.transactions.filter((t:{type:string})=>t.type==='festival_participation')).toHaveLength(1);
 const restored=createPreviewPoints(file);expect(restored.festival('festival_claim').totalReward).toBe(20);expect(restored.festival('festival_claim','free',true).awarded).toBe(false);
...
test('first valid invitation wins; six new registrations award five; existing users, self, copies and retries do not pay',async()=>{
 ...
 expect((await data(host)).invitationReward).toBe(100);expect((await data(host)).successfulInvites).toBe(6);expect((await data(other)).invitationReward).toBe(0);
 ...
 const own=await guests[0].post('/api/v1/festival/claim',{data:{}}).then(r=>r.json());expect(own.participationReward).toBe(20);
 await host.post('/api/v1/festival/claim',{data:{}});const total=await data(host);expect(total.totalReward).toBe(120);expect(total.records).toHaveLength(6);
 await host.post('/api/v1/me/points/check-in',{data:{}});expect((await data(host)).totalReward).toBe(120);expect((await data(host,'/api/v1/me/points')).summary.balance).toBe(165);
...
test('inactive states, write failure and restart preserve transactional consistency',()=>{
 ...
 const upcoming=createPreviewPoints(temporary,{...festivalConfig});expect(upcoming.festival('first').phase).toBe('upcoming');expect(()=>upcoming.festival('first','free',true)).toThrow('activity_inactive');
 ...
 expect(active.festival('first').totalReward).toBe(0);expect(active.festival('first','free',true).totalReward).toBe(20);
 const ended=createPreviewPoints(temporary,{...festivalConfig,enabled:true,endsAt:'2000-01-01T00:00:00Z'});expect(ended.festival('first').phase).toBe('ended');expect(ended.festival('first').totalReward).toBe(20);expect(()=>ended.festival('second','free',true)).toThrow('activity_inactive');
 const state=JSON.parse(readFileSync(temporary,'utf8'));expect(state.wallets.first.pointsBalance).toBe(60);expect(state.festival.rewards.first).toHaveLength(1);
});
...
test('claim login context, wallet/header/ledger synchronization and repeat reload',async({page})=>{
 ...
 await page.goto(origin+'/festival');await page.getByRole('button',{name:'立即参与，领取20积分'}).click();
 ...
 await expect(page.getByRole('button',{name:'已领取20积分'})).toBeVisible();await expect(page.locator('.points-trigger')).toContainText('60');
 await expect(page.locator('.festival-success')).toContainText('20积分已到账');await page.reload();await expect(page.getByRole('button',{name:'已领取20积分'})).toBeVisible();await expect(page.locator('.festival-success')).toHaveCount(0);
...
test('failed response after committed claim recovers without double award',async({page})=>{
 ...
 await page.goto(origin+'/festival');await page.getByRole('button',{name:'立即参与，领取20积分'}).click();
 await expect(page.getByRole('button',{name:'已领取20积分'})).toBeVisible();await expect(page.locator('.festival-success')).toHaveCount(0);
 ...
 expect((await page.request.get(origin+'/api/v1/festival').then(r=>r.json())).totalReward).toBe(20);
});
...
test('responsive layered artwork, all four entry links, clipboard fallback and account isolation',async({page})=>{
 ...
 await page.goto(origin+'/shorts');await page.getByRole('link',{name:'双节福利',exact:true}).click();await expect(page).toHaveURL(origin+'/festival');
 ...
 for(const width of [320,390,768,1440]){await page.setViewportSize({width,height:1000});await expect(page.locator('.festival-title-art')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();
...
test('invitation survives site navigation and registration modal; new user can claim their own reward',async({page})=>{
 ...
 await expect(page.locator('.account-modal')).toHaveCount(0);expect((await data(host)).invitationReward).toBe(20);
 await page.getByRole('link',{name:'双节福利',exact:true}).click();await page.getByRole('button',{name:'立即参与，领取20积分'}).click();await expect(page.locator('.points-trigger')).toHaveText('60');
...
test('two tabs claim once and synchronize; ended/upcoming disable new rewards; reduced motion and touch',async({browser})=>{
 ...
 await Promise.all([a.getByRole('button',{name:'立即参与，领取20积分'}).tap(),b.getByRole('button',{name:'立即参与，领取20积分'}).tap()]);
 await expect(a.locator('.points-trigger')).toHaveText('60');await expect(b.locator('.points-trigger')).toHaveText('60');
 ...
 for(const phase of ['upcoming','ended']){await a.route('**/api/v1/festival',r=>r.fulfill({json:{...result,claimed:false,phase}}));await a.reload();await expect(a.locator('.festival-primary')).toBeDisabled();await expect(a.locator('.festival-secondary')).toBeDisabled();await a.unroute('**/api/v1/festival');}
 await c.close();
});
...
test('late reward response cannot reintroduce the previous account after logout and sign-in',async({page})=>{
 ...
 await page.goto(origin+'/festival');await page.getByRole('button',{name:'立即参与，领取20积分'}).click();await expect.poll(()=>committed).toBe(true);
 await page.locator('.account-avatar-link').click();await page.getByRole('button',{name:'退出账号',exact:true}).click();
 await page.getByRole('link',{name:'双节福利',exact:true}).click();await page.getByRole('button',{name:'立即参与，领取20积分'}).click();
 ...
 release();await expect(page.locator('.account-modal')).toHaveCount(0);await expect(page.locator('.festival-primary')).toContainText('已领取20积分');
 await expect(page.locator('.festival-totals>div').nth(1)).toContainText('0');await expect(page.locator('.points-trigger')).toHaveText('60');
});
